...
## one dimension + one time frame + multiple metrics


TrendedReport <- function(
  rsid,
  start_date,
  end_date,
  dimension,
  ...
  days <- seq.Date(from = as.Date(start_date),
                   to = as.Date(end_date),
                   by = "day")
  
  message(paste0("Beginning queries for date range ", as.character(days[1]), " to ", as.character(days[length(days)]), "."))
  
  df_list <- list()
  for(i in 1:(length(days))){
    message(paste0("Querying data for ", days[i]))
    ...
    while(last_page_check == FALSE){
      query <- paste0('
        {
          ...
          "globalFilters": [
              ...
              {
                  "type": "dateRange",
                  "dateRange": "',days[i],'T00:00:00.000/',days[i]+1,'T00:00:00.000"
              }
          ],
          "metricContainer": {
              "metrics": [',metric_json,']
      ...
      data <- cbind(as.character(days[i]) ,response_df$value, data) 
      names(data) <- c("date", dimension, readable_response$columns$columnIds)
    ...
    df_list[[i]] <- bind_rows(daily_data_list)
    message(paste0("Successfully queried data for ", days[i]))
